...
if \
  [ -z "${GRAASP_APP_HOST}" ] || \
  [ -z "${GRAASP_APP_DEVELOPER_ID}" ] || \
  [ -z "${GRAASP_APP_ID}" ]; then
  echo "error: environment variables GRAASP_APP_GRAASP_APP_ID, GRAASP_APP_GRAASP_DEVELOPER_ID and/or GRAASP_APP_HOST are not defined" 1>&2
...
echo "info: publishing app ${GRAASP_APP_ID} version ${GRAASP_APP_VERSION}"

APP_DIR=${BUCKET}/${GRAASP_APP_DEVELOPER_ID}/${GRAASP_APP_ID}/${GRAASP_APP_VERSION}/

# make sure you do not use the word PATH as a variable because it overrides the PATH environment variable
APP_PATH=${GRAASP_APP_DEVELOPER_ID}/${GRAASP_APP_ID}/${GRAASP_APP_VERSION}
